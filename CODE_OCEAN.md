...
folders to the capsule's corresponding folders. Other top-level files remain
available at the capsule root.

## Environment

Select an R 4.5.x starter environment. The environment must include:
...
During the environment-build stage, restore the pinned R library:

```bash
Rscript -e "if (!requireNamespace('renv', quietly=TRUE)) install.packages('renv', repos='https://cloud.r-project.org'); renv::restore(project='/root/capsule', prompt=FALSE)"
```
